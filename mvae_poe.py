...
class MultiVAE(nn.Module):
    ...
    def forward(self, vision=None, audio=None, tactile=None):
        mu, logvar = self.encoder(vision=vision, audio=audio)
    ...
    def encoder(self, vision=None, audio=None, mu_prior=None, var_prior=None):
        batch_size_current = vision.size(0) if vision is not None else audio.size(0) 
...
def train(args, model, data_loader, optimizer, device='cuda', mu_prior=None, var_prior=None, save_epoch=0):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        vision_recon1, _, mu_vision, logvar_vision = model(vision=vision, audio=None)
        _, audio_recon1,  mu_audio, logvar_audio = model(vision=None, audio=audio)
        ...
        if mu_prior is not None:
            ...
            loss_audio = m_elbo(args=args, recon_vision=None, vision=None,
                                recon_audio=audio_recon, audio=audio, 
                                mu=mu, logvar=logvar, variational_beta=args.variational_beta)
        loss = (loss_joint + loss_vision + loss_audio ) / 3
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
